...
def main(args):
    ...
    for imgId in tqdm.tqdm(imgIds, ncols=100):
        ...
        if args.rare:
            ...
            frequency = lvis.load_cats(ids = category_list)
            rare_class_count = 0
            for frequency_id in range(len(frequency)):
                if frequency[frequency_id]['frequency'] == 'r':
                    rare_class_count+=1
        if args.rare:
            if len(annIds) > 0 and rare_class_count > 0:
...
def get_args():
    ...
    parser.add_argument("--save_dir", default="../../data/RareClasses_LVIS", type=str, help="save images folder")
    ...
    parser.add_argument("--mode", default="P", type=str, help="save mask with rgb or black mode")
    parser.add_argument("--rare", default=True, type=bool, help="get masked images of rare classes")
